 ...
 * @public
 */
	public off(eventName: string, eventCallback: Function): void {
		this.unityEvents = this.unityEvents.filter(
			(event) =>
		...
		if (
			...
			// Check if there are any remaining listeners for this event.
			const hasRemainingListeners = this.unityEvents.some(
